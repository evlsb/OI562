<?php
	...
	$sheet->setCellValue("AD2", "Дебит общего\nконденсата\n(расчет)");
	...
	$nn = 0;
				...
				for ($nn = 0; $nn < $n; $nn++){
					...
					$sheet->setCellValue("W".$num_str, $Mass_Gaz_UVP_Accum[$nn]);	//Масса газа, прошедшая через УИГ
					$sheet->setCellValue("X".$num_str, $WC5_Accum[$nn]);			//Масса WC5+
					$sheet->setCellValue("Y".$num_str, $Mass_water_UIG_Accum[$nn]);	//Масса воды, прошедшя через УИГ
					$sheet->setCellValue("Z".$num_str, $Mass_KG[$nn]);			//Масса КЖ, прошедшая через УИГ
					...
					$sheet->setCellValue("AD".$num_str, $Clean_Cond[$nn]);	//Дебит общего конденсата
					...
					$sheet->setCellValue("AH".$num_str, $Clean_Gaz[$nn]);	//Дебит чистого газа
