...
function Calc_Params(){
	var M_GD     	= document.getElementById("M_GD").value; 		//Масса газа дегазации, кг
	var Dens_GD     = document.getElementById("Dens_GD").value; 	//Плотность газа дегазации, кг/м3
	var V_PO     	= document.getElementById("V_PO").value; 		//Водное число пробоотборника, дм3
	var m_ZO     	= document.getElementById("m_ZO").value; 		//Масса жидкого остатка дегазации, кг
	var Dens_SK     = document.getElementById("Dens_SK").value; 	//Плотность стабильного конденсата, г/см3
	var m_PROB     	= document.getElementById("m_PROB").value; 		//Масса пробы, кг 


	//Вычисляем массу растворенного газа в пробе
	var m_RG = Number(M_GD) + Number(Dens_GD) * (Number(V_PO) - ((Number(m_ZO) - Number(Dens_GD))/(Number(Dens_SK) * 1000 - Number(Dens_GD))));
	//Вычисляем массовую долю растворенного газа
	var W_RG = Number(m_RG) / Number(m_PROB);
...
function Calc_Line(){
  ...
  var confirmm = confirm("Сейчас будет пересчитан текущий замер с новыми значениями ХАЛ! Подтвердите действие!");
  if(confirmm){
	...
	var Number_zamer     	= document.getElementById("Number_zamer").value;	//Номер замера
	var Water_After      	= document.getElementById("Water_After").value;		//Объемная доля воды, %
	var Dens_After       	= document.getElementById("Dens_After").value;		//Плотность нестабильного конденсата, кг/м3
	var DensW_After       	= document.getElementById("DensW_After").value;		//Плотность воды, кг/м3
	...
	var Dens_SK     		= document.getElementById("Dens_SK").value; 		//Плотность стабильного конденсата, г/см3
	var m_PROB     			= document.getElementById("m_PROB").value; 			//Масса пробы, кг
	//var W_HL     			= document.getElementById("W_HL").value; 			//массовая доля хлористых солей, % масс
	//var W_MP     			= document.getElementById("W_MP").value; 			//массовая доля мех. примесей, % масс

	//--------------------Накопленная масса ГЖС (брутто)-----------------------------------------------------------------
	var Mass_brutto_Accum_js   		= document.getElementById("Mass_brutto_Accum").innerHTML; 				//Масса брутто за замер, кг 
	//-------------------------------------------------------------------------------------------------------------------

	//--------------------Для вычисления капельной жидкости, прошедшей через УИГ-----------------------------------------
	var Mass_Gaz_UVP_Accum_js   	= document.getElementById("Mass_Gaz_UVP_Accum").innerHTML; 	//Масса газа, прошедшая через УИГ, кг
	var HAL_iC5H12_js   			= document.getElementById("HAL_iC5H12").value; 	//iC5H12, % мол
	var HAL_nC5H12_js   			= document.getElementById("HAL_nC5H12").value; 	//nC5H12, % мол
	var HAL_C6H14_js   				= document.getElementById("HAL_C6H14").value; 	//C6H14, % мол
	var HAL_C7H16_js   				= document.getElementById("HAL_C7H16").value; 	//C7H16, % мол
	var HAL_H2O_js   				= document.getElementById("HAL_H2O").value; 	//H2O, % мол
	//-------------------------------------------------------------------------------------------------------------------

	//--------------------Накопленный объем газа в УИГ-----------------------------------------------------------------
	var Volume_Count_Forward_sc_Accum_js   		= document.getElementById("Volume_Count_Forward_sc_Accum").innerHTML; 				//Масса брутто за замер, кг 
	//-------------------------------------------------------------------------------------------------------------------

	var Time_measure_js   	= document.getElementById("Time_measure_js").innerHTML; 	//Время замера, сек 
	var Debit_liq_js  		= document.getElementById("Debit_liq_js").innerHTML; 	//Время замера, сек


	var params = 	
							"ServerName=" 		+ ServerName 	+ 				//Имя сервера						
							"&Database=" 		+ Database 		+ 				//имя БД
							"&UID=" 			+ UID 			+ 				//имя пользователя для БД
							"&PWD=" 			+ PWD 			+ 				//пароль

							"&Number_zamer=" 	+ Number_zamer 	+				//Номер замера
							"&Water_After=" 	+ Water_After 	+ 				//Объемная доля воды, %
							"&Dens_After=" 		+ Dens_After 	+ 				//Плотность нестабильного конденсата, кг/м3
							"&DensW_After=" 	+ DensW_After 	+				//Плотность воды, кг/м3

							"&M_GD=" 			+ M_GD 			+ 				//Масса газа дегазации, кг
							"&Dens_GD=" 		+ Dens_GD 		+ 				//Плотность газа дегазации, кг/м3
							"&V_PO=" 			+ V_PO 			+ 				//Водное число пробоотборника, дм3
							"&m_ZO=" 			+ m_ZO 			+ 				//Масса жидкого остатка дегазации, кг
							"&Dens_SK=" 		+ Dens_SK 		+ 				//Плотность стабильного конденсата, г/см3
							"&m_PROB=" 			+ m_PROB		;				//Масса пробы, кг
					
	
	//--------------------Вычисляем массу ГЖС нетто, после замера-----------------------------------------
		var W = (Number(Water_After) * Number(DensW_After)) / Number(Dens_After);			//Массовая доля воды, % масс
		var Mass_netto_Accum   =  Number(Mass_brutto_Accum_js) * (1 - (Number(W) / 100));	//Масса нетто за замер, т
		var Mass_water_Accum   =  Number(Mass_brutto_Accum_js) - Number(Mass_netto_Accum);	//Масса воды в ГЖС за замер, т
	//-------------------------------------------------------------------------------------------------------------------	

	//--------------------Для вычисления капельной жидкости, прошедшей через УИГ-----------------------------------------
		//Накопленная масса WC5+ в УУГ за замер, т
		var Accum_M_WC5_UUG_js = Number(Mass_Gaz_UVP_Accum_js) * ((Number(HAL_iC5H12_js) + Number(HAL_nC5H12_js) + Number(HAL_C6H14_js) + Number(HAL_C7H16_js)) / 100);
		//Накопленная масса воды в УУГ за замер, т
		var Accum_M_H2O_UUG_js = Number(Mass_Gaz_UVP_Accum_js) * (Number(HAL_H2O_js) / 100);
		//Накопленная масса КЖ в УУГ за замер, т
		var Accum_M_KG_UUG_js = Number(Accum_M_WC5_UUG_js) + Number(Accum_M_H2O_UUG_js);
	//-------------------------------------------------------------------------------------------------------------------

	//--------------------Вычисляем накопленную массу конденсата-----------------------------------------
		//Вычисляем массу растворенного газа в пробе 
		var m_RG = Number(M_GD) + Number(Dens_GD) * (Number(V_PO) - ((Number(m_ZO) - Number(Dens_GD))/(Number(Dens_SK) * 1000 - Number(Dens_GD))));
		//Вычисляем массовую долю растворенного газа
		var W_RG = Number(m_RG) / Number(m_PROB);
		//Вычисляем накоп массу газа в ГЖС
		var Mg_GK = Number(Mass_brutto_Accum_js) * (Number(W_RG) / 100);
		//Вычисляем накоп масса конд
		var V_Cond = Number(Mass_brutto_Accum_js) - Number(Mg_GK) + Number(Accum_M_KG_UUG_js) - Number(Mass_water_Accum);
	//-------------------------------------------------------------------------------------------------------------------	

	//--------------------Вычисляем накопленный объем газа-----------------------------------------
		//Вычисляем накоп объем газа в ГЖС
		var Vg_GK = Number(Mg_GK) / Number(Dens_GD);
		//Вычисляем накоп объем чистого газа
		var V_Gaz = Number(Volume_Count_Forward_sc_Accum_js) + Number(Vg_GK); 
	//-------------------------------------------------------------------------------------------------------------------
	

	//Вычисляем массовый расход нетто
	//var M_netto_Rate = Number(Debit_liq_js) * (1 - (Number(W_RG) / 100)) * (1 - (Number(W) + Number(W_HL) + Number(W_MP)) / 100);

	//Накопленная масса воды

	
	//Массовая доля воды, % масс	
	//var W = (Number(Water_After) * Number(DensW_After))/Number(Dens_After); 			
	//alert(Number(ServerName));				
	var params = 	"ServerName=" + ServerName + 
					"&Database=" + Database + 
					"&UID=" + UID + 
					"&PWD=" + PWD + 
					"&Number_zamer=" + Number_zamer + 
					"&Mass_netto_Accum=" + (Number(Mass_netto_Accum)).toFixed(3); 
	//alert(params);
	SendRequest("POST", 'calc_water.php', params, "PrintContent");
	//alert(Mass_netto_Accum.toFixed(3));
...
function Del_Line() {
  ...
  var confirmm = confirm("Выбранный замер будет уделен! Подтвердите действие!");
  ...
  if(confirmm){
	...
	var Number_zamer     = document.getElementById("Number_zamer").value;

	var params = "ServerName=" + ServerName + "&Database=" + Database + "&UID=" + UID + "&PWD=" + PWD + "&Number_zamer=" + Number_zamer; 
	SendRequest("POST", 'delete_row.php', params, "PrintContent");
