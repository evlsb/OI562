<?php
	...
	$nn = 0;
				...
				for ($nn = 0; $nn < $n; $nn++){
					...
					$sheet->setCellValue("O".$num_str, FormatEx($Mass_brutto_Accum[$nn], 3, $NULL));	//Накопленная масса брутто
					$sheet->setCellValue("P".$num_str, FormatEx($Mass_netto_Accum[$nn], 3, $NULL));		//Накопленная масса нетто
					$sheet->setCellValue("Q".$num_str, FormatEx(($Mass_brutto_Accum[$nn] - $Mass_netto_Accum[$nn]), 3, $NULL));				//Накопленная масса воды
					...
					$sheet->setCellValue("U".$num_str, FormatEx($Mg_GK[$nn], 3, $NULL));		//Накопленная масса газа в линии ГЖС
					$sheet->setCellValue("V".$num_str, FormatEx($Vg_GK[$nn], 3, $NULL));		//Накопленный объем газа в линии ГЖС
